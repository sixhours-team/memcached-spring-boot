...
Duration properties such as `expiration` and `expiration-per-cache` by default are using unit of seconds if no unit is specified. `operation-timeout` property by default is using unit of milliseconds if no unit is specified.
